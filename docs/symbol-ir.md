...
This document records Thorn's frontend symbol evidence layer. It sits above the parser-neutral LaTeX frontend and feeds deterministic analysis plus canonical Proof IR elaboration.
...
SymbolTable evidence
        ...
        +--> thorn ir
        |
        `-> canonical Proof IR elaboration
...
The important architectural distinction is that `SymbolTable` is **not the final symbol semantics of canonical Proof IR**. It supplies declarations, uses, scopes, roles, constraints and provenance from which the stronger resolution layer conservatively establishes identity and binding.
...
This is necessary evidence for canonical scope resolution, but a frontend scope record is not by itself permission to merge mathematical objects. Issue #62 established the stronger boundary by making symbol identity, type/domain evidence and binding explicit in Proof IR.
...
These records are observations about the manuscript. Canonical elaboration may establish a binder, domain, type, theorem parameter, witness or substitution target from them when the evidence is sufficient.
...
Canonical resolution strengthens type/domain information only when sufficient evidence exists. Unknown type is a legitimate state.
...
- substitutions and instantiations reference canonical expression nodes rather than rewritten strings.
...
Issue #62 established Proof IR operations involving identity and scope rather than leaving them encoded only as source narration.
...
The exact source remains attached, but the operation does not remain encoded merely as prose once its mathematical meaning is safely recoverable.

The downstream semantic-transformation layer can then use these canonical references for result application and rewriting, and consumer projections such as `thorn-proof/1` can render them without reconstructing identity from surface strings.
...
These limits are intentional. They let canonical Proof IR become stronger without making the evidence layer overconfident.
...
The same rule applies at the other end of the pipeline: symbol semantics must not become model-prompt-specific or Lean-specific merely because those are important consumers. `thorn-proof/1` and the bounded Lean handoff are projections from the canonical identities and operations, not competing symbol systems.
...
The frontend layer should remain conservative and provenance-rich. Stronger notions—canonical identity, binder equivalence, typed parameters, substitution, theorem instantiation and witness provenance—belong in the Proof IR elaboration layer, where ambiguity remains explicit and metamorphic invariants can be tested directly.
