...
Thorn's local NLP layer exists to recover **plausible grammatical evidence**, not to decide mathematical meaning. It is part of the normal frontend used to build rich Math IR for `thorn analyze`, `thorn ir`, and downstream Proof IR elaboration.
...
LaTeX frontend
  ...
  -> canonical typed / partial Proof IR
       |--> thorn-proof/1 / systematic LLM review
...
When stronger independent evidence lets Thorn recover a mathematical operation safely, canonical Proof IR encodes that operation structurally. For example, a linguistic cue may help identify a candidate witness-introduction phrase, but the canonical representation contains a witness operation with symbol identity, obligation and provenance when that interpretation is justified—not a spaCy dependency path.

This is an important anti-regression boundary: do not make canonical Proof IR parser-shaped merely because local NLP was useful in discovering the source relation.
...
For example, a theorem reference grammatically attached inside a proof sentence can be retained as an ambiguous support candidate even when the sentence may only be expository. Later elaboration or semantic review can inspect that evidence rather than rediscovering the paper's structure from scratch.
...
This is also a no-confidence-laundering rule. A linguistic candidate does not become more certain simply because a later layer can render it in a compact or formal-looking syntax.
...
The model-facing representation is no longer merely a future design goal: issue #65 established the stable `thorn-proof/1` projection of canonical Proof IR, with explicit uncertainty, source handles, deterministic fingerprints and a bounded `NEED_SOURCE` rescue contract.

Issue #78 tracks the remaining production integration and evaluation. Until that is complete, `thorn review` still uses the existing semantic-review request path. In either path, spaCy relations are evidence used during construction, not the model-facing semantic vocabulary.

## Relationship to formal proof

The same canonical Proof IR is intended to feed the bounded Lean handoff tracked in #77. Parser-native categories must not leak into generated Lean or be treated as proof evidence by a formal backend.

A formal export may use only the mathematical structure Thorn has mechanically recovered. Linguistically plausible but unresolved content remains an explicit hole or source-linked obligation rather than being guessed into compilable proof code.
