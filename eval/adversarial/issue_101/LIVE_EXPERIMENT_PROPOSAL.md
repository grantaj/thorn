...
The keyless tranche found one deterministic assurance-context loss (A2) and left B0/A1/A3 semantically unresolved because their decisive context reaches the production review boundary. A small live phase would therefore answer a distinct question: when the context is faithful, does the current semantic reviewer actually identify the invariant uniformity/compactness defect, and does A2's impoverished packet produce a materially different result?
...
| A1 | lemma laundering | `lem:uniformize` | `a7b24f8782586dccec61f0833215abcc9568e77ca7136454dcd6cc5d8fc61b15` |
| A2 | prose-defined uniformity | `thm:uniform-decay` | `037375711aab3e2e976b9adbabf25aa61c0706421a7b98cd36f0827bdc36fd40` |
| A3 | theorem-application laundering | `thm:uniform-decay` | `0c8ba6c4a8cbfc2d285384b896e65059f67d296d708f75986deaace3434d22a3` |
...
- Maximum output tokens: **1,500 per request**, including reasoning/output accounting as exposed by the provider. Abort or configure the experiment runner so this is a hard request limit.
- Maximum aggregate output: 15,000 tokens.

The five serialized initial requests observed keylessly total about 24,066 characters, so the 100k input-token guard is deliberately conservative and leaves ample room for bounded rescue packets.

Pricing checked on 2026-08-18 from OpenAI's official GPT-5.6 Sol model documentation: standard text pricing is USD $5.00 / 1M input tokens and $30.00 / 1M output tokens. Under the guards above, the worst-case token charge is therefore at most approximately **USD $0.95** (`100k * $5/M + 15k * $30/M`), excluding any unrelated account/platform charges. Do not use Fast mode for this experiment.
...
Preserve raw provider records in the existing replay format so the experiment becomes exact-replayable without another paid call.

## Discriminating outcomes

The invariant mathematical adjudication is fixed independently of the model.

1. **C0 clean; B0/A1/A3 identify the same compactness/uniformity defect.** This supports the view that the faithful production representation is semantically adequate for these attacks; A2 remains an earlier representation/reachability vulnerability (#125).
...
4. **C0 receives a material false positive.** This is a paired-control failure and blocks any claim that catching defective variants is useful.
5. **All defective cases are missed with faithful packets.** This points to a semantic-review/model assurance gap rather than just #125.
6. **A2 is somehow correctly diagnosed from the impoverished packet.** That does not close #125: the result would rely on model inference/guessing rather than the source-faithfulness invariant, but it is useful empirical evidence about current reviewer robustness.

No adaptive mutation should be made inside the live batch. Any follow-on mutation becomes a separately frozen experiment with new hashes/fingerprints and separate authorization.
