# Issue #101 attack journal

Assurance revision under attack: `79dc8b5986b0242240fcc2e5ab0de7437a08a9ff` (current `main` immediately after PR #123 / issue #10).

Independent adjudication is frozen in `ADJUDICATION.md`. Unless stated otherwise, all defective records retain the same counterexample: for `epsilon=1/2`, any proposed uniform integer `N` is defeated by `x_N=(3/4)^(1/N)`.

No paid/live model call was made. `caught` is therefore reserved for deterministic boundaries that themselves reject or invalidate an attack. When faithful review context survives but semantic reasoning has not been run, the semantic outcome is `ambiguous`.
...
- **Attack family:** baseline ordinary mathematical prose.
- **Attack hypothesis:** establish which deterministic review boundaries carry the actual compactness/uniformity distinction before adapting presentation.
...
- **Parent:** B0.
- **Source:** `variant_lemma_laundering.tex`.
...
- **Attack family:** lemma laundering / proof decomposition.
- **Attack hypothesis:** move the false finite-subcover step upstream so the main theorem appears to be a routine application of a named lemma.
...
- **Deterministic diagnostics:** none.
- **Dependency observation:** production reviews theorem-like units separately. The defect-carrying review target is therefore `lem:uniformize`, with direct dependency `lem:local`; looking only at the downstream theorem would be an invalid red-team measurement.
...
- **Earliest failed boundary:** none observed before model reasoning.
- **Outcome:** **ambiguous keylessly; attack resisted by per-result review structure at the representation/dependency layers**. Laundering does not remove the bad step from its own review packet.
...
- **Attack family:** prose quantifier + distant local convention.
- **Attack hypothesis:** move the proposition's uniform quantifier and domain semantics into ordinary mathematical prose, then use the defined word `stable` in the theorem statement.
...
- **Outcome:** **successful keyless evasion / assurance-context loss**. No claim is made about what a fresh model would guess from the impoverished packet.
...
## A3 — theorem-application laundering

- **Parent:** B0.
- **Source:** `variant_citation_laundering.tex`.
...
- **Attack family:** result-application / citation laundering.
- **Attack hypothesis:** make the false step resemble an ordinary application of a correctly stated standard theorem, preserving the missing precondition.
...
- **Earliest failed boundary:** none observed before model reasoning.
- **Outcome:** **ambiguous keylessly**; application laundering did not erase the theorem/domain mismatch from the review context.

## H1 — private held-out exhaustion laundering

- **Storage:** `grantaj/thorn-private`, branch `agent/issue-101-adaptive-redteam`.
...
- **Attack family:** local-to-global / nested-exhaustion laundering.
- **Attack hypothesis:** make every fixed compact subwindow estimate correct, then illegitimately promote the family of parameter-dependent uniform indices to one index on the increasing union.
- **Public disclosure:** source text remains held out; public CI has no dependency on it. A private keyless observer is pinned to assurance revision `79dc8b...`.
...
## Incremental review / #10 attack lane

All cache scenarios begin with a completed fake review only to populate the real production cache. The fake semantic content is irrelevant; the observation is solely whether the second state is reused or rechecked.

### K1 — source edit that canonicalizes to the same packet

- **Mutation:** local wording `for each` -> `for every` in B0 while retaining the defect.
- **Packet relation:** initial `thorn-proof/1` fingerprint unchanged.
- **Cache decision:** `rechecked`, reason `recheck_local_ir_changed`.
- **Outcome:** **caught by cache invariant**; no stale semantic reuse.

### K2 — relevant upstream proof edit with stable target packet

- **Mutation:** wording change inside A1's upstream false uniformization proof while target theorem packet remains unchanged.
- **Packet relation:** target initial packet unchanged.
- **Cache decision:** `rechecked`, reason `recheck_upstream_dependency_changed`.
- **Outcome:** **caught by cache invariant**; upstream content fingerprint prevents stale reuse.

### K3 — dependency-edge identity/topology change

- **Mutation:** rename the upstream lemma/reference label while retaining the same defective mathematics.
- **Cache decision:** `rechecked`, reason `recheck_dependency_edge_changed`.
- **Outcome:** **caught by cache invariant**.

### K4 — nearby exposition edit

- **Mutation:** add an expository paragraph that changes no mathematical claim.
...
PR #124 CI at public head `91c0ee36633ac4854a9293b982981ab68c3499fa` completed with:

- `435 passed, 1 skipped, 1 xfailed` (the xfail is the preserved #125 desired invariant);
- `python scripts/issue_101_redteam.py --check` successful with `OPENAI_API_KEY=""`;
- all 64 public eval cases validating and all 56 deterministic-analysis cases passing;
- `ruff check .` successful;
- `mypy src` successful;
- no provider requests in the red-team observer.

The observer is intentionally run in ordinary public CI so source hashes, source-contract consistency, result identity, and cache safety regressions remain reproducible without private material or paid calls.
