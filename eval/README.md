...
This directory contains small, self-contained regression cases for Thorn's deterministic checker and semantic audit.
...
The same corpus now specifies two deliberately different capabilities:

```text
thorn check   deterministic structural analysis
...
Existing semantic cases default to both modes. A structural fixture may use:
...
Deterministic expectations live in `eval/check-expectations.json`. That manifest must cover every check-enabled case exactly. An empty rule list is a real expectation: the checker must stay silent on that case at its current capability level.
...
- **L6 — cross-result dependency:** a theorem depends on a flawed lemma elsewhere in the paper, including cases where the downstream theorem happens to be true but its stated proof is unsupported.
...
Run the complete deterministic matrix with exact rule expectations:
...
5. Run the relevant deterministic or semantic mode and observe the failure.
...
Default CI is deliberately offline. It runs unit tests, fixture validation, the complete deterministic matrix, linting, and type checking with `OPENAI_API_KEY` explicitly blank. Those checks must never make billable model calls.
