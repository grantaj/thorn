...
The remaining distinction is a **review-policy/model-boundary semantic judgement over an already faithful packet**. The current reviewer contract already asks for actual mathematical defects and explicitly forbids converting parser/recovery uncertainty into correctness findings. Because issue #98 authorizes no live model calls, this tranche does not provide experimental evidence that the versioned reviewer prompt itself causes systematic ambient-assumption false positives or silent strengthening. The frozen prompt is therefore intentionally unchanged rather than being speculatively tuned.

The public regression family makes counterfactual materiality an explicit evaluation category and mechanically verifies that clean and defect contexts remain distinguishable at the review boundary. A future measured semantic-review evaluation can test whether prompt policy needs a separately versioned revision without changing Proof IR or historical experiment evidence.
