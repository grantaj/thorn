...
def _parser() -> argparse.ArgumentParser:
    ...
    mode.add_argument("--preflight", action="store_true", help="construct exact request keylessly")
    mode.add_argument("--live", action="store_true", help="make one paid synthetic provider call")
...
def main() -> int:
    ...
    if args.replay is not None:
        ...
        response = verify_readiness_evidence(evidence)
        _write_json(
            args.output,
            {
                "format": "thorn-provider-readiness-replay/1",
    ...
    if args.live:
        ...
        evidence = run_live_readiness(args.model)
    ...
    evidence = preflight_readiness(args.model)
    _write_json(args.output, evidence.model_dump(mode="json"))
    return 0
