...
def _categories(path: Path) -> list[AnalysisCategory]:
    return [finding.category for finding in analyze_project(extract_project(path))]


def test_dependency_analysis_covers_duplicates_ambiguity_missing_and_cycles(tmp_path: Path) -> None:
...
""",
    ...
    findings = analyze_project(extract_project(tex))
    categories = {finding.category for finding in findings}

    assert AnalysisCategory.DUPLICATE_LABEL in categories
    assert AnalysisCategory.AMBIGUOUS_REFERENCE in categories
    assert AnalysisCategory.MISSING_REFERENCE in categories
    assert AnalysisCategory.CIRCULAR_DEPENDENCY in categories
...
""",
    ...
    assert AnalysisCategory.MISSING_REFERENCE not in _categories(tex)
...
""",
    ...
    assert analyze_project(extract_project(tex)) == []
...
""",
    ...
    assert AnalysisCategory.ROLE_CONFLICT in _categories(bad)
...
""",
    ...
    assert AnalysisCategory.ROLE_CONFLICT not in _categories(clean)
